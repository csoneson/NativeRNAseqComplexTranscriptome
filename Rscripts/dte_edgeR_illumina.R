...
(des <- model.matrix(~ group, data = metadata))

## Filter out transcripts with average CPM below 0.5
print(dim(dge0))
cpms <- cpm(dge0)
dge <- dge0[apply(cpms, 1, mean) > 0.5, ]
dge <- calcNormFactors(dge)
print(dim(dge))

## Add transcript annotation
annot <- tx2gene %>% dplyr::select(-start, -end) %>% distinct()
if (any(duplicated(annot$tx))) {
  stop(paste0("The following transcripts are represented by multiple rows in the ", 
              "annotation: ", 
              paste(annot$tx[duplicated(annot$tx)], collapse = ",")))
}
annot <- annot[match(rownames(dge), annot$tx), ]
rownames(annot) <- annot$tx
dge$genes <- annot
...
(contrasts <- as.data.frame(makeContrasts(SprkVsWT = groupWT, levels = des)))
